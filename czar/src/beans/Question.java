		...
		public void setTest(String testName) throws ClassNotFoundException, SQLException
			{
				...
				databaseConnection.executeQuery("select count(*)"
						+ " from Test_Tables where TestName='" + testName + "'");
				databaseConnection.resultSet.next();
				listOfAvailableTables = new String[Integer
						.parseInt(databaseConnection.resultSet.getString(1))];
				databaseConnection.executeQuery("select * from Test_Tables "
						+ "where TestName='" + testName + "'");
				int counter = 0;
				while (databaseConnection.resultSet.next())
					{
						listOfAvailableTables[counter++] = databaseConnection.resultSet
								.getString("TableName");
					}
