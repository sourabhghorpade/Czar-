...
<body>
	...
	if (S.equals("Register")) {
		...
		try {
			ResultSet resultSet;
			resultSet=databaseConnection.executeQuery("select * from student where username='"+ U +"' and password='" + P + "'");
			if(resultSet.next())
			{
								
				user.setUsername(U);
				user.setPassword(P);
				user.setMin(Integer.parseInt(resultSet.getString("min")));
				user.setSec(Integer.parseInt(resultSet.getString("sec")));
				user.setName(resultSet.getString("names"));
				user.setScore(Integer.parseInt(resultSet.getString("score")));
				user.setCno(resultSet.getString("cno"));
				user.setYr(resultSet.getString("yr"));
				databaseConnection.disconnect();
				%>
				<script>
